...
%	\autoheight
		...
		\column{0.495\textwidth}
			...
			\includegraphics[width=0.8\textwidth]{./Figures/mssa_msse_zc.pdf}
				\leftskip2em \singlespacing{ {\footnotesize \emph{Note:}{Financial Crisis\\Nowcasts: M-SSA (blue) and M-MSE (green). \\The vertical green lines indicate sign changes of M-MSE  \\M-SSA (blue) has less zero-crossings (smoother).
				\label{mssa_msse_now}}}}
		\end{center}
		\end{columns}
...
\end{footnotesize}
\begin{singlespace}
	{\footnotesize \emph{Notes:}  Out-of-sample rRMSE of M-SSA vs. expanding mean forecast of GDP. Out-of-sample evaluation starting in Q1 2008, without pandemic.\\Values $< 1$ indicate superior predictions by M-SSA.}
\end{singlespace}
\vspace{0.5cm}
\end{table}
...
\end{footnotesize}
\begin{singlespace}
	{\footnotesize \emph{Notes:} p-values for $H_0: \hat{\beta_1} = 0$, HAC-adjusted Wald test, Equation \ref{eq_gdp_pred}.\\Sample excludes pandemic.}
\end{singlespace}
\vspace{0.5cm}
\end{table}
