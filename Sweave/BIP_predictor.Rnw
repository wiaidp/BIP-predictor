...
par(mfrow=c(1,2))
k<-3
plot(lags[,1,length(select_vec_multi)],acf_values[,1,k], type = "h", main = paste("GDP vs. ",select_vec_multi[k],sep=""), xlab = "Lag", ylab = "ACF")
abline(h = 0, col = "red")
k<-4
plot(lags[,1,length(select_vec_multi)],acf_values[,1,k], type = "h", main = paste("GDP vs. ",select_vec_multi[k],sep=""), xlab = "Lag", ylab = "ACF")
abline(h = 0, col = "red")
...
par(mfrow=c(1,2))
  ts.plot(mplot_with_pandemic,col=colo_i,main=paste("MA inversion GDP: with pandemic",sep=""),xlab="lag")
  for (k in 1:length(select_vec_i))
    mtext(select_vec_i[k],line=-k,col=colo_i[k])
    ts.plot(mplot_without_pandemic,col=colo_i,main="Without pandemic",xlab="lag")
#  for (k in 1:length(select_vec_i))
#    mtext(select_vec_i[k],line=-k,col=colo_i[k])
...
mat1<-as.matrix(tail(data_table))
...
cat("\\begin{center}")
cat("\\includegraphics[height=3in, width=4.5in]{", file, "}\n",sep = "")
cat("\\caption{Sample cross-correlation function: GDP and ifo (left), GDP and ESI (right): full data set, including the pandemic. Correlations at positive lags mean that real-time GDP is lagging in relative terms.", sep = "")
cat("\\label{CCF}}", sep = "")
cat("\\end{center}")
cat("\\end{figure}")
@
...
cat("\\begin{center}")
cat("\\includegraphics[height=3in, width=4.5in]{", file, "}\n",sep = "")
cat("\\caption{Sample cross-correlation function: GDP and ifo (left), GDP and ESI (right): without Pandemic. Correlations at positive lags mean that real-time GDP is lagging in relative terms.", sep = "")
...
cat("\\begin{figure}[H]")
cat("\\begin{center}")
cat("\\includegraphics[height=4in, width=5in]{", file, "}\n",sep = "")
...
cat("\\begin{center}")
cat("\\includegraphics[height=4in, width=5in]{", file, "}\n",sep = "")
cat("\\caption{Direct forecast  across the financial crisis:  regression based on all indicators and full sample information.", sep = "")
...
cat("\\caption{Shifted GDP (black), direct HP forecast (red) and classic direct forecast (blue) over the curse of the financial crisis:  regression based on all indicators and full sample information (without pandemic).", sep = "")
...
colnames(mat_re)<-paste("h=",0:3,sep="")
mat1<-round(mat_re,3)
#latex(cor_vec, dec = 1, , caption = "Example of using latex to create table",
#center = "centering", file = "", floating = FALSE)
...
ts.plot(mplot,col=colo,main="M-MSE",xlab="")
for (i in 1:ncol(mplot))
  mtext(colnames(mplot)[i],col=colo[i],line=-i)
mplot<-bk
colo<-rainbow(ncol(mplot))
ts.plot(mplot,col=colo,main="M-SSA",xlab="")
for (i in 1:ncol(mplot))
  mtext(colnames(mplot)[i],col=colo[i],line=-i)
#mplot<-bk_HP_HP_C
#colo<-rainbow(ncol(mplot))
...
dim(mplot)
colnames(mplot)<-c("Target: HP-BIP","M-SSA","M-MSE")
...
mat_cor_ht<-rbind(c(cor(na.exclude(mplot[,c("Target: HP-BIP","M-SSA")]))[1,2],
...
cat("\\begin{center}")
cat("\\includegraphics[height=3in, width=4.5in]{", file, "}\n",sep = "")
cat("\\caption{Two-sided HP(160) applied to BIP (target, black) and nowcasts:  M-SSA (blue) and  M-MSE (green). The dashed vertical line delimits in- and out-of-sample spans. The first L=31 observations are lost due to initialization and the two-sided filter does not extend to the sample end.", sep = "")
...
#  mplot<-scale(cbind(forward_shifted_BIP,hp_c_array_wc[select,,h+1],predictor_mssa_mat_wc[,h+3]))
  ...
  colnames(mplot)<-c("BIP","M-SSA","HP-C")
...
cat("\\begin{center}")
cat("\\includegraphics[height=3in, width=4.5in]{", file, "}\n",sep = "")
cat("\\caption{Now- and forecasts of HP-BIP for $h=0,...,6$: HP-C (top) vs. M-SSA (bottom) across the financial crisis (left) and the pandemic (right).", sep = "")
...
cat("\\begin{center}")
cat("\\includegraphics[height=3in, width=4.5in]{", file, "}\n",sep = "")
cat("\\caption{Forward-shifted BIP (black), M-SSA (blue) and HP-C (red) for $h=3,4,5,6$ across the financial crisis. All series are standardized for better visual inspection.", sep = "")
