...
L<-31
# Publication lag
# -In contrast to M-SSA package (tutorial 7.4) we here assume a lag of one quarter only (instead of two)
# -To replicate the results in package we then have to impose a forecast excess of 5 to BIP (instead of 4)
#   -M-SSA estimation routine shifts HP-BIP by lag[1]+f_excess[1]+h
...
# Filter the indicators: apply HP-C
hp_c_mat<-NULL
...
p_value_HAC_BIP_oos_wc<-perf_obj$p_value_HAC_HP_BIP_oos_wc

p_value_HAC_BIP_oos_wc
...
mat1<-round(p_value_HAC_BIP_oos_wc,3)
