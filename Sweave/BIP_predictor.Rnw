...
cat("\\begin{center}")
cat("\\includegraphics[height=3in, width=4.5in]{", file, "}\n",sep = "")
cat("\\caption{Nowcasting: two-sided HP(160) applied to BIP (target, black),  M-SSA (blue) and and M-MSE (green). The dashed vertical line delimits in- and out-of-sample spans. The first L=31 observations are lost due to initialization and the two-sided filter does not extend to the sample end.", sep = "")
...
paste("Target correlations and HTs of M-SSA and M-MSE nowcasts.  "),
...
cat("\\begin{center}")
cat("\\includegraphics[height=3in, width=4.5in]{", file, "}\n",sep = "")
cat("\\caption{Forward-shifted BIP (black), M-SSA (blue) and HP-C (red) for $h=3,4,5,6$ across the financial crisis.", sep = "")
