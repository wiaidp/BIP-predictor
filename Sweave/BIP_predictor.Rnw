...
par(mfrow=c(1,2))
ts.plot(mplot_with_pandemic,col=colo_i,main=paste("MA inversion GDP: with pandemic",sep=""),xlab="")
  for (k in 1:length(select_vec_i))
  { 
    if (k==1)
    {
...
ts.plot(mplot_without_pandemic,col=colo_i,main="Without pandemic",xlab="")
#  for (k in 1:length(select_vec_i))
#    mtext(select_vec_i[k],line=-k,col=colo_i[k])
...
for (h in 0:3)
{
  ...
  p_val_hp<-c(p_val_hp,pf(sum_obj$fstatistic["value"], sum_obj$df[1],sum_obj$df[2], lower.tail = FALSE))
...
if (F)
{
# rRMSEs are computed for h=0:6 (just increase h in the above lopp to recompute)
# Compare MSEs and p-values
  rrmse_hp_direct<-sqrt(MSE_hp_vec/MSE_unfiltered_vec)
  names(rrmse_hp_direct)<-names(p_val_hp)<-names(p_val_unfiltered)<-paste("h=",0:6,sep="")
  
  save(rrmse_hp_direct,file=paste(path.result,"/rrmse_hp_direct",sep=""))
  save(p_val_unfiltered,file=paste(path.result,"/p_val_unfiltered",sep=""))
  save(p_val_hp,file=paste(path.result,"/p_val_hp",sep=""))
} else
{
  load(file=paste(path.result,"/rrmse_hp_direct",sep=""))
  load(file=paste(path.result,"/p_val_unfiltered",sep=""))
  load(file=paste(path.result,"/p_val_hp",sep=""))
...
mat1<-matrix(round(rrmse_hp_direct[4:7],5),nrow=1)
colnames(mat1)<-names(rrmse_hp_direct)[4:7]
rownames(mat1)<-"rRMSE"
#latex(cor_vec, dec = 1, , caption = "Example of using latex to create table",
#center = "centering", file = "", floating = FALSE)
xtable(mat1, dec = 1,digits=rep(3,dim(mat1)[2]+1),
paste("Relative root mean-square error of HP-C against classic direct forecast: numbers smaller one signify an outperformance by the direct HP-C forecast.  "),
label=paste("f_stat",sep=""),
center = "centering", file = "", floating = FALSE)
@
...
mat1<-matrix(round(p_val_unfiltered,5),nrow=1)
colnames(mat1)<-names(p_val_unfiltered)
rownames(mat1)<-"P-values"
#latex(cor_vec, dec = 1, , caption = "Example of using latex to create table",
#center = "centering", file = "", floating = FALSE)
xtable(mat1, dec = 1,digits=rep(3,dim(mat1)[2]+1),
paste("Statistical significance of direct forecasts.  "),
label=paste("f_stat",sep=""),
center = "centering", file = "", floating = FALSE)
@
...
mplot<-bk
colo<-rainbow(ncol(mplot))
ts.plot(mplot,col=colo,main="M-SSA: h=0",xlab="")
for (i in 1:ncol(mplot))
  mtext(colnames(mplot)[i],col=colo[i],line=-i)
...
mplot<-bk
colo<-rainbow(ncol(mplot))
ts.plot(mplot,col=colo,main="M-SSA: h=4",xlab="")
#for (i in 1:ncol(mplot))
#  mtext(colnames(mplot)[i],col=colo[i],line=-i)
...
ts.plot(mplot,col=colo_i,main="MA inversion GDP: 2008",xlab="")
for (k in 1:length(select_vec_i))
  mtext(select_vec_i[k],line=-k,col=colo_i[k])
...
ts.plot(mplot,col=colo_i,main="Full data set",xlab="")
#for (k in 1:length(select_vec_i))
#  mtext(select_vec_i[k],line=-k,col=colo_i[k])
...
cat("\\begin{center}")
cat("\\includegraphics[height=3in, width=5in]{", file, "}\n",sep = "")
cat("\\caption{M-SSA filter tracking HP-GDP at forecast horizon $h=4$: data up to Jan-2008 (left) vs. entire data set (right).", sep = "")
