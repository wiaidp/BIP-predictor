...
# Extended time series
library(pROC)
# Elastic net for estimating VAR
library(elasticnet)
...
# Elastic net functions for VAR estimation
source(paste(getwd(),"/R/elastic_net_functions.r",sep=""))
...
# This is the setting used in the paper
VAR_type="VAR"
#VAR_type="BVAR"
#VAR_type="elastic_net"
if (VAR_type=="BVAR")
{
...
{
# Marc's settings for EN
  q<-0
# 1. Simpler (but possibly too simple)  
  p<-2
  nsteps<-5
# 2. Simpler than Simon's original settings (below) but slightly more complex than above ones  (good compromise)
  p<-3
  nsteps<-6
# Simon's original settings (lead to overfitting)  
#  p<-4
#  nsteps<-7
}
...
# Default (better): use_garch<-F (otherwise important crises are underweighted)
use_garch<-F
...
# Full sample estimation with/without pandemic
...
# Apply regularization: see vignette MTS package
  ...
  if (VAR_type=="BVAR")
  {
    ...
    V_obj <- BVAR(data_fit,p = p,C = C_BVAR,V0 = V0_BVAR)
    save(V_obj,file=paste(path.result,"/BVAR",sep=""))
  }
  if (VAR_type=="elastic_net")
  {
    V_obj<-enetVAR(data_fit, p, L, nsteps  )

    save(V_obj,file=paste(path.result,"/en_VAR",sep=""))
  }
...
# Apply regularization: see vignette MTS package
    threshold<-1.5
    V_obj_wc<-refVARMA(V_obj, thres = threshold)
  ...
  if (VAR_type=="BVAR")
  {
    ...
    #   low values <=> low precision
    C_BVAR <- lambda_BVAR*diag(rep(1,n*p+1))
    V_obj_wc <- BVAR(data_fit,p = p,C = C_BVAR,V0 = V0_BVAR)
    save(V_obj_wc,file=paste(path.result,"/BVAR_wc",sep=""))
  } 
  if (VAR_type=="elastic_net")
  {
    V_obj_wc<-enetVAR(data_fit, p, L, nsteps  )

    save(V_obj_wc,file=paste(path.result,"/en_VAR_wc",sep=""))
  }
...
} else
{
  ...
  if (VAR_type=="BVAR")
  {
    load(file=paste(path.result,"/BVAR",sep=""))
    load(file=paste(path.result,"/BVAR_wc",sep=""))
  }
  if (VAR_type=="elastic_net")
  {
    load(file=paste(path.result,"/en_VAR",sep=""))
    load(file=paste(path.result,"/en_VAR_wc",sep=""))
  }
}
...
rownames(mat2)<-as.character(index(data_table))[(nrow(data_table)-nrow(mat2)+1):nrow(data_table)]
#latex(cor_vec, dec = 1, , caption = "Example of using latex to create table",
#center = "centering", file = "", floating = FALSE)
...
# Table with empirical HT and target correlations
...
# Plot emphasizing zero-crossings towaards financial crisis
file = "./Figures/mssa_msse_zc.pdf"
pdf(file = paste(path.out,file,sep=""), paper = "special", width = 7, height = 7)
...
colo<-c("black","violet","blue","orange","green")
colo<-c("blue","green")
main_title<-"M-MSE & M-SSA Nowcasts: Financial Crisis"
plot(mplot[,1],main=main_title,axes=F,type="l",xlab="",ylab="",col=colo[1],lwd=1,ylim=c(min(na.exclude(mplot)),max(na.exclude(mplot))))
mtext(colnames(mplot)[1],col=colo[1],line=-1)
for (i in 1:ncol(mplot))
{
...
#   -The function computes M-SSA for each forecast horizon h in h_vec
  mssa_indicator_obj<-compute_mssa_BIP_predictors_func(x_mat,lambda_HP,L,date_to_fit,p,q,ht_mssa_vec,h_vec,f_excess,lag_vec,select_vec_multi,nsteps,VAR_type,lambda_BVAR)
# Without pandemic  
  mssa_indicator_obj_wc<-compute_mssa_BIP_predictors_func(x_mat_wc,lambda_HP,L,date_to_fit,p,q,ht_mssa_vec,h_vec,f_excess,lag_vec,select_vec_multi,nsteps,VAR_type,lambda_BVAR)
  if (VAR_type=="VAR")
  {
  ...
  if (VAR_type=="BVAR")
  {
    save(mssa_indicator_obj,file=paste(path.result,"/BV_mssa_indicator_obj",sep=""))
    save(mssa_indicator_obj_wc,file=paste(path.result,"/BV_mssa_indicator_obj_wc",sep=""))
  }
  if (VAR_type=="elastic_net")
  {
    save(mssa_indicator_obj,file=paste(path.result,"/en_mssa_indicator_obj",sep=""))
    save(mssa_indicator_obj_wc,file=paste(path.result,"/en_mssa_indicator_obj_wc",sep=""))
  }
...
} else
{
  ...
  if (VAR_type=="BVAR")
  {
    load(file=paste(path.result,"/BV_mssa_indicator_obj",sep=""))
    load(file=paste(path.result,"/BV_mssa_indicator_obj_wc",sep=""))
  }
  if (VAR_type=="elastic_net")
  {
    load(file=paste(path.result,"/en_mssa_indicator_obj",sep=""))
    load(file=paste(path.result,"/en_mssa_indicator_obj_wc",sep=""))
  }
}
# Target series: output of two-sided HP applied to BIP: 
...
# The following double loop computes all combinations of forward-shifts (of BIP) and forecast horizons (of M-SSA)
  for (shift in shift_vec)#shift<-4
  {
...
# The results can be saved (overwritten)    
  ...
  if (VAR_type=="BVAR")
  {
    save(list_perf,file=paste(path.result,"BV_list_perf",sep=""))
  }
  if (VAR_type=="elastic_net")
  {
    save(list_perf,file=paste(path.result,"en_list_perf",sep=""))
  }
} else
{
# Load all results
  ...
  if (VAR_type=="BVAR")
  {
    load(file=paste(path.result,"BV_list_perf",sep=""))
  }
  if (VAR_type=="elastic_net")
  {
    load(file=paste(path.result,"en_list_perf",sep=""))
  }
...
if (recompute_results)
{
  final_mssa_indicator_obj<-compute_mssa_BIP_predictors_func(x_mat_wc,lambda_HP,L,date_to_fit_all,p_full,q_full,ht_mssa_vec,h_vec,f_excess,lag_vec,select_vec_multi,nsteps,VAR_type,lambda_BVAR)
  ...
  if (VAR_type=="BVAR")
  {
    save(final_mssa_indicator_obj,file=paste(path.result,"/BV_final_mssa_indicator_obj",sep=""))
  }
  if (VAR_type=="elastic_net")
  {
    save(final_mssa_indicator_obj,file=paste(path.result,"/en_final_mssa_indicator_obj",sep=""))
  }
} else
{
  if (VAR_type=="VAR")
  ...
  if (VAR_type=="BVAR")
  {
    load(file=paste(path.result,"/BV_final_mssa_indicator_obj",sep=""))
  }
  if (VAR_type=="elastic_net")
  {
    load(file=paste(path.result,"/en_final_mssa_indicator_obj",sep=""))
  }
...
if (reg_type=="Ridge")
{
  lm_obj<-lm.ridge(dat[,1]~matrix(cbind(rep(1,ncol(dat)-1),dat[,2:ncol(dat)]),ncol=ncol(dat))-1, lambda = lambda_reg)
}
if (reg_type=="Lasso")
{
  lm_obj<-glmnet(x=dat[,2:ncol(dat)],y=dat[,1],alpha=0,lambda = lambda_reg)
}
...
# VAR based on full data set without pandemic  
  ...
  if (VAR_type=="BVAR")
  {
    ...
    #   low values <=> low precision
    C_BVAR <- lambda_BVAR*diag(rep(1,n*p+1))
    V_obj_full <- BVAR(data_fit,p = p,C = C_BVAR,V0 = V0_BVAR)
    save(V_obj_full,file=paste(path.result,"/BVAR_full",sep=""))
  }
  if (VAR_type=="elastic_net")
  {
    V_obj_full<-enetVAR(data_fit, p, L, nsteps  )

    save(V_obj_full,file=paste(path.result,"/enVAR_full",sep=""))
  }
...
# VAR based on data pre 2008  
  ...
  if (VAR_type=="BVAR")
  {
    ...
    #   low values <=> low precision
    C_BVAR <- lambda_BVAR*diag(rep(1,n*p+1))
    V_obj_2008 <- BVAR(data_fit,p = p,C = C_BVAR,V0 = V0_BVAR)
    save(V_obj_2008,file=paste(path.result,"/BVAR_2008",sep=""))
  }
  if (VAR_type=="elastic_net")
  {
    V_obj_2008<-enetVAR(data_fit, p, L, nsteps  )

    save(V_obj_2008,file=paste(path.result,"/enVAR_2008",sep=""))
  }
...
} else
{
  ...
  if (VAR_type=="BVAR")
  {
    load(file=paste(path.result,"/BVAR_full",sep=""))
    load(file=paste(path.result,"/BVAR_2008",sep=""))
  }
  if (VAR_type=="elastic_net")
  {
    load(file=paste(path.result,"/enVAR_full",sep=""))
    load(file=paste(path.result,"/enVAR_2008",sep=""))
  }
...
select_direct_indicator<-select_vec_multi
# Select data matrix: full sample without pandemic
data_roc<-x_mat_wc
...
shift_vec<-1:5
hh_vec<-1:6
AUC_array_GDP<-AUC_array_HP_GDP<-array(dim=c(length(shift_vec),length(hh_vec),4))
dimnames(AUC_array_GDP)<-dimnames(AUC_array_HP_GDP)<-list(paste("shift=",shift_vec,sep=""),paste("h=",hh_vec,sep=""),c("Direct forecast","Direct HP forecast","M-MSE","M-SSA"))
...
# Apply smoothing    
    ...
    AUC<-ROCplots(ROC_data, showROC , main = paste("shift=",shift,", h=",h,sep=""), lbls = lbls,
                  ...
                  showLegend , lg_cex = lg_cex, lg_ncol = 1)
    AUC_array_GDP[i,j,]<-unlist(AUC)
  }
}
#invisible(dev.off())
...
#file = "./Figures/ROC_HP_GDP_many.pdf"
#pdf(file = paste(path.out,file,sep=""), paper = "special", width = 7, height = 7)
par(mfrow=c(length(which(shift_vec>k)),length(which(hh_vec>k))))
showl<-T
for (i in 1:length(shift_vec))#i<-1
...
# Apply smoothing    
    ...
    AUC<-ROCplots(ROC_data, showROC , main = paste("shift=",shift,", h=",h,sep=""), lbls = lbls,
                  smoothROC , colours =NULL, lwd = 2,
                  showLegend , lg_cex = lg_cex, lg_ncol = 1)
    AUC_array_HP_GDP[i,j,]<-unlist(AUC)
  }
}
#invisible(dev.off())
...
file = "./Figures/ROC_GDP_shift_1_4.pdf"
pdf(file = paste(path.out,file,sep=""), paper = "special", width = 7, height = 7)
par(mfrow=c(1,2))
...
file = "./Figures/ROC_HP_GDP_shift_1_4.pdf"
pdf(file = paste(path.out,file,sep=""), paper = "special", width = 7, height = 7)
par(mfrow=c(1,2))
...
# Select plots that will be shown    
showLegend<-T
AUC<-ROCplots(ROC_data, showROC , main = paste("HP-GDP: shift=",shift,sep=""), lbls = lbls,
                  smoothROC , colours =NULL, lwd = 2,
                  showLegend , lg_cex = 1, lg_ncol = 1)
shift<-4
h<-shift
...
AUC_array_GDP["shift=1",,]
AUC_array_GDP["shift=2",,]
AUC_array_GDP["shift=3",,]
AUC_array_GDP["shift=4",,]
AUC_array_GDP["shift=5",,]

# Compute table with diagonal (shift=h)
auc_GDP_mat<-auc_HP_GDP_mat<-NULL
for (i in 1:length(shift_vec))
{
  auc_GDP_mat<-rbind(auc_GDP_mat,AUC_array_GDP[i,i,])
  auc_HP_GDP_mat<-rbind(auc_HP_GDP_mat,AUC_array_HP_GDP[i,i,])
}
rownames(auc_GDP_mat)<-rownames(auc_HP_GDP_mat)<-paste("Forecast horizon ",shift_vec,sep="")
auc_GDP_mat
auc_HP_GDP_mat
...
cat("\\begin{center}")
cat("\\includegraphics[height=3in, width=5in]{", file, "}\n",sep = "")
cat("\\caption{Hit rate vs. false alarm rate of the predictors at small (one quarter ahead:left panel) and yearly forecast horizons (right panel).", sep = "")
cat("\\label{ROC_GDP_shift_1_4}}", sep = "")
cat("\\end{center}")
cat("\\end{figure}")
@
...
mat1<-round(auc_GDP_mat,3)
#latex(cor_vec, dec = 1, , caption = "Example of using latex to create table",
#center = "centering", file = "", floating = FALSE)
xtable(mat1, dec = 1,digits=rep(3,dim(mat1)[2]+1),
paste("AUCs of predictors against GDP at various forecast horizons."),
