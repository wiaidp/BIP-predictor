...
f_vec_unfiltered<-p_val_unfiltered<-NULL
for (h in 0:3)
{
...
par(mfrow=c(2,2))
f_vec_hp<-t_mat_hp<-p_val_hp<-NULL
for (h in 0:3)
{
  # Shift BIP forward by publication lag+forecast horizon
  forward_shifted_BIP<-c(x_mat_wc[(1+lag_vec[1]+h):nrow(x_mat_wc),"BIP"],rep(NA,h+lag_vec[1]))
...
mat_re<-rbind(p_val_unfiltered,p_val_hp)
...
paste("P-values of of F-statistics of direct forecasts and direct HP forecasts for forecast horizons $h=0,1,2,3$: fully in-sample.  "),
label=paste("f_stat",sep=""),
center = "centering", file = "", floating = FALSE)
@
