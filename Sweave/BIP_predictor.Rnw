...
\[GDP_{t+shift}=\beta_0^{shift,h,filt}+\sum_{i=1}^5\beta_i^{shift,h,filt}y_{ti}^{h,filt}+\epsilon_{t}^{h,shift,filt}=\hat{GDP}_{t+shift}^{filt}+\epsilon_{t}^{shift,filt},\]
where $y_{ti}^{h,filt}$ represents the $i$-th filtered indicator, based on either HP-C, M-MSE, or M-SSA. For simplicity of exposition, we present results based on the full sample, excluding the pandemic period. Fig. 
<<label=init,results=hide>>=
...
# Select plots that will be shown    
showLegend<-T
AUC<-ROCplots(ROC_data, showROC , main = paste("shift=",shift,sep=""), lbls = lbls,
                  smoothROC , colours =NULL, lwd = 2,
                  showLegend , lg_cex = 1, lg_ncol = 1)
shift<-4
h<-shift
...
#center = "centering", file = "", floating = FALSE)
xtable(mat1, dec = 1,digits=rep(3,dim(mat1)[2]+1),
paste("AUCs of different predictors against GDP at various forecast horizons.."),
