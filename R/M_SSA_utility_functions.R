...
# Check forward-shifts: HP-BIP cannot reach sample-end since filter is two-sided
  ...
  p_value_HAC_HP_BIP_oos_wc<-t_HAC_HP_BIP_full<-p_value_HAC_HP_BIP_full<-t_HAC_HP_BIP_oos<-p_value_HAC_HP_BIP_oos<-cor_mat_HP_BIP_full<-cor_mat_HP_BIP_oos<-matrix(ncol=length(h_vec),nrow=length(h_vec)-1)
...
# Same but out of sample 
      ...
      oos_index_wc<-which(rownames(predictor_mssa_mat)>date_to_fit&rownames(predictor_mssa_mat)<"2020")
      
      da<-na.exclude(cbind(target_shifted_mat[,i],predictor_mssa_mat[,j])[oos_index_wc,])
      
      # Compute HAC-adjusted p-value of-one-sided test when regressing column 2 (predictor) on column 1 (target) of da
      p_obj<-HAC_ajusted_p_value_func(da)
      
      p_value_HAC_HP_BIP_oos_wc[i,j]<-p_obj$p_value
      
      
    }
  }
  colnames(p_value_HAC_HP_BIP_full)<-colnames(t_HAC_HP_BIP_full)<-colnames(cor_mat_HP_BIP_full)<-
  colnames(p_value_HAC_HP_BIP_oos)<-colnames(t_HAC_HP_BIP_oos)<-colnames(cor_mat_HP_BIP_oos)<-
    colnames(p_value_HAC_HP_BIP_oos_wc)<-paste("M-SSA: h=",h_vec,sep="")
  rownames(p_value_HAC_HP_BIP_full)<-rownames(t_HAC_HP_BIP_full)<-rownames(cor_mat_HP_BIP_full)<-
  rownames(p_value_HAC_HP_BIP_oos)<-rownames(t_HAC_HP_BIP_oos)<-rownames(cor_mat_HP_BIP_oos)<-
    rownames(p_value_HAC_HP_BIP_oos_wc)<-paste("Shift of target: ",h_vec[-length(h_vec)],sep="")
...
# Same but out of sample 
  ...
  rownames(p_value_HAC_BIP_oos)<-rownames(t_HAC_BIP_oos)<-rownames(cor_mat_BIP_oos)<-
  paste("Shift of target: ",h_vec[-length(h_vec)],sep="")
...
# Chexk forwrad-shift    
  ...
  return(list(p_value_HAC_HP_BIP_oos_wc=p_value_HAC_HP_BIP_oos_wc,p_value_HAC_HP_BIP_full=p_value_HAC_HP_BIP_full,t_HAC_HP_BIP_full=t_HAC_HP_BIP_full,
...
#   on volatility obtained from a GARCH(1,1) model fitted to target (first column of dat)
    if (use_garch)
    {
      weight<-garch_vola_func(dat,shift,lag_vec)$weight[1:i]
    } else
    {
# Fixed weight      
      weight<-rep(1,i)
